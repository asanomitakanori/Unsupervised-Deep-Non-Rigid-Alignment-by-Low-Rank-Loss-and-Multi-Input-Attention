...
from val import Val
from Model.model import LowRankNet as Net
...
def train_net(model, 
    ...
    if cfg.eval.imgs is not None:
        train = abs_path(cfg.train.imgs)
        val = abs_path(cfg.eval.imgs)
        n_train = len(train)
        n_val = len(val)

    epochs = cfg.train.epochs
    batch_size = cfg.train.batch_size
    train_loader = MnistLoadTest(train)
    val_loader = MnistLoadTest(val)
    ...
    for epoch in range(epochs):
        global_step = Train(train_loader, model, device, optimizer, global_step, writer)
        val_loss = Val(val_loader, model, device, global_step)
